...
### Tous les algorithmes implémentés en Python (pour formation)

Ces implémentations sont à des fins de démonstration. 
Elles sont moins efficaces que les implémentations de la bibliothèque 
standard Python.
...
**Le tri à bulles**, parfois appelé *tri par enfoncement*, est un algorithme 
de tri simple qui parcourt à plusieurs reprises la liste à trier, compare 
chaque paire d’articles adjacents et les échange s’ils sont dans le mauvais 
ordre. Le passage dans la liste est répété jusqu’à ce qu’aucun swap ne soit
nécessaire, ce qui indique que la liste est triée.

__Propriétés__
* Rendement le plus défavorable	O(n<sup>2</sup>)
* Rendement au meilleur des cas	O(n)
* Rendement moyen des cas	O(n<sup>2</sup>)

###### Source: [Wikipedia][bubble-wiki] 
###### Afficher l’algorithme dans [action][bubble-toptal]

### Bucket
![alt text][bucket-image-1]
![alt text][bucket-image-2]

**Bucket sort**, ou _bin sort_, est un algorithme de tri qui fonctionne en
distribuant les éléments d’un tableau dans un certain nombre de buckets. 
Chaque seau est ensuite trié individuellement, soit en utilisant un 
algorithme de tri différent, soit en appliquant récursivement l’algorithme
de tri du seau.

__Propriétés__
* Rendement le plus défavorable	O(n<sup>2</sup>)
* Rendement au meilleur des cas	O(n+k)
* Rendement moyen des cas	O(n+k)
...
**Tri du shaker à cocktail**, aussi connu sous le nom de _ bulle bidirectionnelle
sort_, _cocktail sort_, _shaker sort_ (qui peut également faire référence à une
variante de _selection sort_), _ripple sort_, _shuffle sort_, ou _shuttle sort_,
est une variation du tri à bulles qui est à la fois un algorithme de tri stable 
et un tri de comparaison. L’algorithme diffère d’un tri à bulles en ce qu’il
trie dans les deux directions à chaque passage à travers la liste.
...
**Tri d’insertion** est un algorithme de tri simple qui construit le tableau
(ou la liste) trié final un élément à la fois. Il est beaucoup moins efficace 
sur les *grandes* listes que les algorithmes plus avancés tels que quicksort,
heapsort ou merge sort.
...
###### Afficher l’algorithme dans [action][insertion-toptal]


### Tri fusion
![alt text][merge-image]

**Le tri de fusion** (aussi souvent orthographié *mergesort*) est un algorithme
de tri efficace, général et fondé sur la comparaison. La plupart des 
implémentations produisent un tri stable, ce qui signifie que l’implémentation 
préserve l’ordre d’entrée des éléments égaux dans la sortie triée. Mergesort 
est un algorithme diviser pour régner qui a été inventé par John von Neumann 
en 1945.

__Propriétés__
* Rendement le plus défavorable	O(n log n)
* Rendement au meilleur des cas	O(n log n)
* Rendement moyen des cas	O(n log n)

###### Source: [Wikipedia][merge-wiki] 
###### Afficher l’algorithme dans[action][merge-toptal]
